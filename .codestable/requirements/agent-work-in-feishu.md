...
implemented_by: [2026-05-16-lark-cli-wrapper, 2026-05-17-config-yaml, 2026-05-18-hooks-merge, 2026-05-18-roostery-init, 2026-05-18-bot-task-writer, 2026-05-18-bot-stop-hook, 2026-05-18-init-real-lark-cli-override, 2026-05-19-bot-bridge-cluster]
...
- **2026-05-18** ⭐：`bot-stop-hook` 落地（feature `2026-05-18-bot-stop-hook`，commit `220c7b0`，CI run `26030808131` 全绿），**Phase 5 Module F 第 2 子 feature = minimal-loop closing = 0.1.0 触发判据达成**。本次 `draft → current` 升级——req 的"agent 跑完在飞书出现"用户视角**真正端到端跑通**：CC SessionEnd → 极简 sh wrapper → `roostery bot stop-hook` (Rust 端原生 stdin JSON + transcript jsonl tail + IM 兜底) → 飞书 task 卡 + step stream + 多设备同步可见。**额外兑现一维**：新增反向 CLI `roostery bot push --agent X --session Y --summary "..." --json --strict`——让任意 agent / 脚本 / cron / CI 主动把进度推到飞书，不限于 stop hook 被动触发；这是"vendor-neutral broker"定位（用户故事第 3 条 "Claude Code / Codex / Gemini 之类"）的具体兑现层，覆盖未来用户接入任意自定义 agent runtime 的场景。**Rust 红利显式发挥**：类型化 `PushRequest` builder + `PushOutcome` 结构化 `--json` 输出（v1 稳定契约）+ `--strict` opt-in 真实 exit code + blake3 稳态 idempotency key + structured tracing——这些都是 Python parity 学不来的工程质量。**保留原始愿景不动**：vibecoding 多设备协作 / 数据主权落用户自家飞书 / 不锁定其他 IM 呈现 / 不替代 agent runtime 等边界全部原文保留。**下一阶段**：(a) 真机 dogfood（跑 `roostery init` + CC 一轮 prompt + 手机飞书看 task 卡 + 群消息 thread）；(b) 0.1.0 release 准备（README / CHANGELOG / crates.io，归 brainstorm `v0.x-direction` 决议）；(c) Phase 5 第 3 子 feature `bot-bridge-cluster` 把 IM thread / hitl_router / bot_relay_task 等扩展能力接上

- **2026-05-18**（同日补丁）：`init-real-lark-cli-override` 落地（feature `2026-05-18-init-real-lark-cli-override`，commit `aa06807`，CI run `26036982700` 全绿），根治 issue `2026-05-18-init-shim-conflicts-npm-prefix`——onboarding init 在 npm 全局 prefix == shim target (`~/.local/bin/lark-cli`) 时 `RealLarkCliMissing` fail，且 live 模式失败留 install_shim 破损态的 P2 缺陷。**这是 req 的 B 用户首次装机入口关键缺口修复**——bot-stop-hook accept 时真机 dogfood 第一次撞此问题（npm install -g @larksuite/cli 是最常见装法）；本 feature 加 `--real-lark-cli <PATH>` flag + 复用 `ROOSTERY_LARK_CLI_BIN` env 作 init-time override + resolve 调用上移到 F1 后早 gate + 错误信息拆 3 sub-variant 含 fix hint。**真机 dogfood 完整跑通**（mv npm symlink + roostery init --real-lark-cli + shim transparent forward + journal 自动落档 + 模拟 CC SessionEnd → 真飞书出 task `d4e8c06f-...`），**兑现 bot-stop-hook accept 跳过的被动 hook 真飞书那半 dogfood**。**req status 保持 `current`**——本 feature 是修补不是新能力；用户故事 / 边界 / pitch 全部原文不动。**下一阶段**：(a) 0.1.0 release 准备（README / CHANGELOG / crates.io）；(b) 真用户跑 CC headless 端到端实测；(c) shim self-loop 检测改进（issue O2 留观察项）

- **2026-05-18** 🎯：`release-0.1.0-prep` 落地（feature `2026-05-18-release-0.1.0-prep`），**首个 0.1.0 release 形态达成**。version bump `0.0.0 → 0.1.0` + `git tag v0.1.0`（本地，push 时机用户自决）+ README 五段重写（hero 把多设备 vibecoding 痛点 + 数据主权前置，技术属性下移）+ CHANGELOG.md 起步（Keep a Changelog 1.1.0 格式，17 feature highlights 按 Phase 0-5 排）+ workspace Cargo.toml metadata 7 字段补齐（description / keywords / categories / readme / homepage / documentation / rust-version）为 0.2.0 crates.io 预热。**这是 req 的"对外门面"层兑现**——B 类用户首次接触 Roostery 看到的 README 不再是 placeholder，而是 user-why leading 的能力陈述；0.1.0 milestone 在 git ref 留痕。**req 用户故事 / 边界 / pitch 原文不动**——本 feature 是文档 + metadata 工作，不改愿景。**0.1.x 不上 crates.io**（brainstorm `v0.x-direction` 决议），cargo 字段为 0.2.0 publish 预热。**下一阶段**：(a) 用户决定 push v0.1.0 tag 时机；(b) Phase 5 第 3 子 feature `bot-bridge-cluster`；(c) 0.2.0 准备时跑 `cargo publish --dry-run`

- **2026-05-19**：`bot-bridge-cluster` 落地（feature `2026-05-19-bot-bridge-cluster`），**Phase 5 Module F 第 3 子 feature**。**长跑 daemon `roostery bot bridge`** 订阅多 bot IM event 流，匹配 @mention 触发 runner 跑、写飞书 task step、IM thread 回复；群里 `/stop` `/abort` `停` `中止` 反向中止正在跑的 agent，`/adjust <body>` 带追加 prompt 重启 runner（上限 1 次）。**兑现用户故事第 4 条**：协作中的团队成员"打开飞书就能围观、点评、接续"——此前 bot-stop-hook 只把 stop hook 结果推到飞书是被动单向写，本 feature 把 IM 群消息变成反向控制 agent 的入口，**飞书从被动 view 升级为指挥面**。**架构层兑现**："Rust 期重新设计而非 Python 1:1 翻译"代表案例：5 Python 模块（bot_role / bot_runner / bot_bridge / bot_relay_task / hitl_router）合并 1 Rust 子目录 9 文件；**HITL 反向控制走进程内 tokio oneshot channel 不落盘 sentinel**（ARCHITECTURE.md §5 新增第 9 条架构决定）。**红线对齐**：飞书 IO 必经 LarkRunner trait（IM streaming subscribe 走变量注入 binary 路径不触发 grep）；本地缓存 `~/.roostery/state/bot_chats/` 是 chat→TaskGuid 映射不是真相；0 LLM client import。**req status 保持 `current`**——本 feature 是协作维度的扩展兑现，用户故事 / 边界 / pitch 原文不动；vibecoding 多设备协作 / 数据主权 / 不锁定其他 IM 呈现等核心愿景保留。**下一阶段**：(a) 真用户在飞书群里 @bot 端到端 dogfood；(b) Phase 6 Module G `report-git-llm` + `report-daily`（日报 / docx / Base 记录）；(c) Phase 7 `base-indexer` + `legacy-removal` 终态切换
