...
- `git log` 在**已初始化但 0 commit 的仓库**（`git init` 后从未 `git commit`）退 **128** 而非 0 + stderr `"your current branch '<name>' does not have any commits yet"`——不是"非 git 仓"错误（那条 stderr 是 `"not a git repository"`，也 exit 128）。调用方要按 stderr substring 区分：`"not a git repository"` → 真错；`"does not have any commits yet"` → 兜底成空 commit list 继续。参考 `crates/roostery/src/daily_recap/git_log.rs::collect_repo`（report-recap-engine feature 2026-05-19 踩到）
