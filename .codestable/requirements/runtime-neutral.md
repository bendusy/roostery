...
implemented_by: [2026-05-18-dispatcher-trace-budget, 2026-05-18-dispatcher-rules, 2026-05-18-dispatcher-runners, 2026-05-18-dispatcher-loop]
...
- **2026-05-18**：`dispatcher-loop` 落地（feature `2026-05-18-dispatcher-loop`），**Phase 4 Module E 收尾子 feature——整个 Module E 完成**。串 trace / budget / runaway / rules / runners / journal 6 上游模块为 `HookEvent in → DispatchOutcome out + journal` 主链路；暴露 `roostery dispatcher fire / replay / test-rule` 三 CLI 子命令；emitted_events 链式自触发分发（BFS 队列 + trace.max_depth + DEFAULT_MAX_FANOUT=16 双守门）；fire 始终 exit 0 + journal 落档失败；replay live 真跑 runner + 分配新 trace_id；unknown runner kind → StepStatus::Skipped 与 req "新 runtime 接入前用户感知 not supported" 边界一致。**dispatcher 编排层完全不感知具体 runtime**——这是 req 在 dispatcher 这一侧的最终兑现。**req 仍保持 `draft`**——用户端到端感知"换 runtime 飞书侧呈现不变"需要 Phase 5 bot-task-writer + bot-stop-hook 把 dispatcher.fire 真正接到飞书 task 卡片输出，由 Phase 5 收尾 acceptance 一次性升级 `draft → current`
